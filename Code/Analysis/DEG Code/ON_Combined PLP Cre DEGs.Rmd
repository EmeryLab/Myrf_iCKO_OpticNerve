---
title: "Optic Nerve PLPCre+ DEG list"
...
md[, .N, by = c("combined.genotype", "named.clusters")] %>% kable()
...
#Creating DEG list
markers <- FindMarkers(ON_Combined, ident.1 = "PLP Cre+" , ident.2 = "Control", group.by = "combined.genotype", subset.ident = ON_Combined_list[i], logfc.threshold = 0.322)

if(is_empty(markers)) {next}
...
assign(paste("PLPCre", names(ON_Combined_list)[i], sep = "_"), markers , envir = DEG.env)
...
save(list = list_objects, file = here("Outputs", "DEGs" ,"ON PLP Cre+ Defining DEGs.RData"), envir = DEG.env)
...
write.xlsx(alldf, sheetName = names(alldf), file = here("Outputs", "DEGs", "ON PLP Cre+ Defining DEGs.xlsx"))
```
