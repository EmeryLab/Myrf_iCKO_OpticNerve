...
DimPlot(ON.OL, reduction = "umap", cols=natparks.pals("Torres"))

DimPlot(ON.OL, reduction = "umap", group.by = "Replicate", cols=natparks.pals("GrandCanyon", n=7))

DimPlot(ON.OL, reduction = "umap", group.by = "combined.genotype", cols=natparks.pals("Yellowstone"))
DimPlot(ON.OL, reduction = "umap", split = "combined.genotype", cols=natparks.pals("Torres"))

table(Idents(ON.OL))

table(Idents(ON.OL), ON.OL$Replicate)

table(Idents(ON.OL), ON.OL$combined.genotype)
```
...
# Psuedolineage Analysis Using Monocle3
...
# PLP Psuedolineage Analysis Using Monocle3
```{r echo=TRUE, warning=FALSE, error=FALSE, results = FALSE, message=FALSE, fig.align = 'center'}

ON.OL.PLP <- subset(x = ON.OL, subset = combined.genotype == c("PLP Cre+"))
DimPlot(ON.OL.PLP, reduction = "umap", group.by = "combined.genotype", cols = natparks.pals("Yellowstone"))

# convert to cell_data_set object (cds.OL)
cds.OL.PLPv2 <- as.cell_data_set(ON.OL.PLP)
...
# Run this to determine the `time_bin` for the seurat_cluster of interest
DimPlot(ON.OL.PLP, reduction = "umap", group.by = "seurat_clusters", label = T)

# a helper function to identify the root principal points:
get_earliest_principal_node <- function(cds.OL.PLPv2, time_bin= "19"){
...
# Sox10 Psuedolineage Analysis Using Monocle3
```{r echo=TRUE, warning=FALSE, error=FALSE, results = FALSE, message=FALSE, fig.align = 'center'}
ON.OL.Sox10 <- subset(x = ON.OL, subset = combined.genotype == c("Sox10 Cre+"))
DimPlot(ON.OL.Sox10, reduction = "umap", group.by = "combined.genotype", cols = natparks.pals("Yellowstone"))

# convert to cell_data_set object (cds.OL)
cds.OL.Sox10v2 <- as.cell_data_set(ON.OL.Sox10)
...
# Run this to determine the `time_bin` for the seurat_cluster of interest
DimPlot(ON.OL.Sox10, reduction = "umap", group.by = "seurat_clusters", label = T)

# a helper function to identify the root principal points:
get_earliest_principal_node <- function(cds.OL.Sox10v2, time_bin= "19"){
